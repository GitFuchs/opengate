...
void lookUpTable::readLookUpTable(const std::string &strFilename)
{
    ...
    // std::cout << "found columns: " << columns << " found rows: " << rows << std::endl;
    ...
    // std::cout << "number of vector before reading: " << data.size() << std::endl;
...
bool lookUpTable::verifyLUT()
{
    ...
    for (long unsigned int i = 0; i < data.size(); i++)
    {
        ...
        {
            std::cout << "ERROR: row " << i << " has " << data[i].size() << " columns, but should have " << columns << std::endl;
            return false;
        }
    }
    ...
    for (long unsigned int i = 1; i < data.size(); i++)
    {
        ...
        {
            std::cout << "ERROR: row " << i << " has a smaller value than row " << i - 1 << std::endl;
            return false;
        }
    }
    ...
    {
        std::cout << "ERROR: not enough columns in look up table. Number of columns found: " << columns << std::endl;
        return false;
    }
...
/* bool lookUpTable::findEntriesByLet(int charge, double let, std::vector<double> &result)
{
    ...
    auto lower_charge = std::lower_bound(data.begin(), data.end(), charge, [this](auto &vec, int value)
                                         { return vec[charge_column] < value; });

    auto upper_charge = std::upper_bound(data.begin(), data.end(), charge, [this](int value, auto &vec)
                                         { return value < vec[charge_column]; });
    ...
    if (lower_charge == data.end())
    {
        ...
        {
            result.push_back(0.);
            // std::cout << "j: " << j << "to acchieve " << data[i].size() << "size: " << (int)result.size() << "capacity: " << (int)result.capacity() << std::endl;
            return false;
        }
    }
    ...
    // check if no suitable charge was found, in this case lower und upper_charge should be the same, e.g. at end.
    if (lower_charge == upper_charge)
    {
        std::cout << "WARNING: no suitable charge entry found for charge: " << charge << std::endl;
        // exit(-1);
        for (long unsigned int j = 0; j < data[0].size() - key_columns; j++)
        {
            result.push_back(0.);
    ...
    auto lower_let = std::lower_bound(lower_charge, upper_charge, let, [this](auto &vec, double value)
                                      { return vec[let_column] < value; });
//     // auto upper_let = std::upper_bound(lower_charge, upper_charge, let, [this](double value, auto &vec)
...
//     // exit(-1);
    ...
    if (*(lower_let->begin() + let_column) == let)
    {
        ...
        std::copy(data[i].begin() + key_columns, data[i].end(), std::back_inserter(result));
        return true;
    }
    // // if let too low and not in list, return first one
    else if ((*(lower_charge->begin() + let_column) > let) && (lower_let == lower_charge))
    {
        // convert iterator to index
    ...
    else if ((*(std::prev(upper_charge)->begin() + let_column) < let) && (lower_let == upper_charge))
    {
        // convert iterator to index
    ...
    else
    {
        ...
        if (lower_let > lower_charge)
        {
            ...
            //           << " first let: " << *((lower_let - 1)->begin() + let_column)
            //           << " second let: " << *((lower_let)->begin() + let_column)
            //           << " indexed let: " << *(lower_let->begin() + let_column)
            //           << std::endl;

            // std::cout << "in, so range valid" << std::endl;
            result = interpolateVectors(let, let_column, *std::prev(lower_let), *(lower_let));
            return true;
        }
    }
...
bool lookUpTable::findEntriesByEnergy(int charge, double energy, std::vector<double> &result)
{
    // returns 0 if successful
    ...
    if (lower_charge == data.end())
    {
        std::cout << "INFO: no suitable charge entry found for charge: " << charge << std::endl;
        for (long unsigned int j = 0; j < data[0].size() - key_columns; j++)
        {
            result.push_back(0.);
            // std::cout << "j: " << j << "to acchieve " << data[i].size() << "size: " << (int)result.size() << "capacity: " << (int)result.capacity() << std::endl;
    ...
    // ##############################
    // find range of iterators in which the charge is correct
    auto lower_energy = std::lower_bound(lower_charge, upper_charge, energy, [this](auto &vec, double value)
                                         { return vec[energy_column] < value; });
    ...
    // safety check if end of lut was exceeded, if so go to last entry which is in LUT
    if (lower_energy > data.end())
    {
        lower_energy = data.end();
        std::cout << "WARNING: Look-up table: reseting index. This should not happen." << std::endl;
    }
    ...
    if (*(lower_energy->begin() + energy_column) == energy)
    {
        ...
        //           << " previous energy: " << *(std::prev(lower_energy)->begin() + energy_column)
        //           << " indexed energy: " << *(lower_energy->begin() + energy_column)
        //           << " returned energy: " << *(lower_energy->begin() + energy_column)
        //           << std::endl;
        return true;
    }
    // // if energy too low and not in list, return first one
    else if ((*(lower_charge->begin() + energy_column) > energy) && (lower_energy == lower_charge))
    {
        // convert iterator to index
        i = std::distance(data.begin(), lower_charge);
        std::copy(data[i].begin() + key_columns, data[i].end(), std::back_inserter(result));
    ...
    else if ((*(std::prev(upper_charge)->begin() + energy_column) < energy) && (lower_energy == upper_charge))
    {
        // convert iterator to index
        i = std::distance(data.begin(), std::prev(upper_charge));
        std::copy(data[i].begin() + key_columns, data[i].end(), std::back_inserter(result));
    ...
    else
    {
        if (lower_energy > lower_charge)
        {
            ...
            // std::cout << "in, so range valid" << std::endl;
            result = interpolateVectors(energy, energy_column, *std::prev(lower_energy), *(lower_energy));
            return true;
        }
    }
    ...
    std::cout << "NOT FOUND target energy: " << energy
              //   << " previous energy: " << *(std::prev(lower_energy)->begin() + energy_column)
              //   << " indexed energy: " << *(lower_energy->begin() + energy_column)
              //   << " returned energy: " << *(upper_charge->begin() + energy_column)
              << std::endl;
...
std::vector<double> lookUpTable::interpolateVectors(double targetValue, int column_to_use, std::vector<double> vector1, std::vector<double> vector2)
{
    ...
    {
        resultValue = interpolateValue(targetValue, vector1[column_to_use], vector2[column_to_use], vector1[i], vector2[i]);
        result.push_back(resultValue);
    }
    return result;
}

double lookUpTable::interpolateValue(double target, double x_1, double x_2, double f_1, double f_2)
{
    double result;

    result = f_1 + (f_2 - f_1) / (x_2 - x_1) * (target - x_1);
    // std::cout << "interpol target " << target << " x_1 " << x_1 << " x_2 " << x_2 << " f_1 " << f_1 << " f_2 " << f_2 << " result " << result << std::endl;
    if ((result <= f_2 && result >= f_1) || (result >= f_2 && result <= f_1))
    {
    }
    else
    {
        std::cout << "+++++++++++++++++++Interpolation WARNING++++++++++++++++++++++++++++++++++++" << std::endl;
        std::cout << "in interpolateValue: target " << target << " x_1 " << x_1 << " x_2 " << x_2 << " f_1 " << f_1 << " f_2 " << f_2 << " result " << result << std::endl;
    }
    return result;
}

void lookUpTable::determineRowsAndColumnsFromFile(const std::string &strFilename, int &rows, int &columns)
{
    rows = 0;
    columns = 0;
    ...
    if (!inputfile.is_open())
    {
        std::cout << "Error opening file: " << strFilename << " exiting." << std::endl;
        exit(-1);
    }
    ...
    {
        columns_in_line = 0;
        // std::getline(inputfile, line); // read the first line of your file to string
        ...
        double value;
        while (iss >> value)
            columns_in_line++; // while there's something in the line, increase the number of columns
    ...
    // inputfile.seekg(0);
    // columns = max_columns;
    // // std::cout << "found columns: " << columns_in_line << " found rows: " << rows << std::endl;
