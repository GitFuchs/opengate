...
public:
  ...
  // lookUpTable(const int rows, const int columns);                 // Empty
  // Matrix With Defined Size lookUpTable(const int rows, const int columns,
  // const T **data); // Defined Matrix
  ...
  void determineRowsAndColumnsFromFile(const std::string &strFilename,
                                       int &rows, int &columns);
  std::vector<double> interpolateVectors(double targetValue, int column_to_use,
                                         std::vector<double> vector1,
                                         std::vector<double> vector2);

  double interpolateValue(double target, double x_1, double x_2, double f_1,
                          double f_2);
  // bool findEntriesByLet(int charge, double let, std::vector<double> &result);
  bool findEntriesByEnergy(int charge, double energy,
                           std::vector<double> &result);
  bool verifyLUT();

  // bool findEntries(int charge, int atomic_mass, double let,
  // std::vector<double> &result);
