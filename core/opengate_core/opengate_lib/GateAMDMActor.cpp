    ...
    : GateVActor(user_info, true)
{
  ...
  // std::cout
  //     << "AMDM image size: " << size[0] << " " << size[1] << " " << size[2] << " " << size[3] << std::endl;
  ...
  {
    std::cout << "The file name for the AMDM LUT is empty. Aborting." << std::endl;
    exit(-1);
  }
...
void GateAMDMActor::ActorInitialize(py::dict &user_info)
{
  ...
  famdm_tables->readLookUpTable(fLUTfilename);
  famdm_tables->printLookUpTable();
  std::cout << "AMDM actor initialized"
            << std::endl;
}
...
void GateAMDMActor::BeginOfRunAction(const G4Run *)
{
  ...
  AttachImageToVolume<ImageType4D>(cpp_amdm_gamma_image, fPhysicalVolumeName,
                                   fInitialTranslation);
  //   AttachImageToVolume<  AttachImageToVolume<ImageType4D>(cpp_amdm_restricted_edep_image, fPhysicalVolumeName,
  // >(cpp_amdm_delta_image, fPhysicalVolumeName,
  //                                        fInitialTranslation);
  //   AttachImageToVolume<  AttachImageToVolume<ImageType4D>(cpp_amdm_restricted_edep_image, fPhysicalVolumeName,
  // >(cpp_amdm_gamma_image, fPhysicalVolumeName,
  //                                        fInitialTranslation);
  AttachImageToVolume<ImageType>(cpp_amdm_restricted_edep_image, fPhysicalVolumeName,
                                 fInitialTranslation);
...
void GateAMDMActor::SteppingAction(G4Step *step)
{
  ...
  bool isInside = cpp_amdm_restricted_edep_image->TransformPhysicalPointToIndex(point, index);
  ...
  if (isInside)
  {
    ...
    // skip if charge is smaller or equal to zero, as outside of applicability of MCF MKM model
    if (charge <= 0)
      return;

    // double cut = DBL_MAX;
    // important add units !
    // G4double density = step->GetTrack()->GetMaterial()->GetDensity() / (CLHEP::g / CLHEP::cm3);
    ...
    {
      // std::cout << "kinetic Energy: " << kinEnergy << " atomic mass: " << atomic_mass << std::endl;
      return;
    }
    // std::cout << "edep: " << edep << "kinetic Energy: " << kinEnergy << " atomic mass: " << atomic_mass << std::endl;

    bool success = false;
    std::vector<double> entry;
    // famdm_tables->printLookUpTable();
    // std::cout << "starting lookup" << std::endl;
    success = famdm_tables->findEntriesByEnergy(charge, kinEnergy / atomic_mass, entry);
    // first kMaxAMDMBins entries for gamma, second kMaxAMDMBins entries for delta
    // std::cout << "entry size: " << entry.size() << std::endl;
    // std::cout << "entry 0: " << entry[0] << std::endl;
    // std::cout << "success is not yet there" << std::endl;
    if (success)
    {
      // std::cout << "success is true" << std::endl;
      // store restricted edep
      ImageAddValue<ImageType>(cpp_amdm_restricted_edep_image, index, edep);
      // std::cout << "added edep to edep image with index: " << index << std::endl;
      ...
      // std::cout << "Dimension of cpp_amdm_delta_image " << cpp_amdm_delta_image->GetLargestPossibleRegion().GetSize() << std::endl;
      // std::cout << "Dimension of cpp_amdm_gamma_image " << cpp_amdm_gamma_image->GetLargestPossibleRegion().GetSize() << std::endl;
      // std::cout << "Origin of cpp_amdm_delta_image" << cpp_amdm_delta_image->GetOrigin() << std::endl;
      // std::cout << "Origin of cpp_amdm_gamma_image" << cpp_amdm_gamma_image->GetOrigin() << std::endl;
      // std::cout << "Spacing of cpp_amdm_delta_image" << cpp_amdm_delta_image->GetSpacing() << std::endl;
      // std::cout << "Spacing of cpp_amdm_gamma_image" << cpp_amdm_gamma_image->GetSpacing() << std::endl;
      ...
      for (int i = 0; i < kMaxAMDMBins; i++)
      {
        ...
        // BinnedPixel_delta_e[i] = delta_e;
        // BinnedPixel_gamma_d[i] = gamma_d;

        // std::cout << " charge : " << charge << " kin energy " << kinEnergy
        //           << " gamma LUT: " << entry[i] << " delta LUT: " << entry[i + secondHalf]
        //           << " edep: " << edep << " gamma_d : " << gamma_d << " delta_e : " << delta_e << std::endl;
        // // std::cout << " i: " << i << " entry[i] " << entry[i] << " bin j :" << j << " gamma_d hist: " << j << " delta_d hist: " << j + secondHalf << std::endl;
        // std::cout << " i " << i << " edep: " << edep << " gamma_d : " << gamma_d << " delta_e : " << delta_e << std::endl;
      }
      // // ############## filling
      // ImageAddValue<VectorImageType>(cpp_amdm_delta_image, index, BinnedPixel_delta_e);
      // ImageAddValue<VectorImageType>(cpp_amdm_gamma_image, index, BinnedPixel_gamma_d);
    }
    else
    {
      std::cout << "INFO: no data found in LUT for particle: charge: " << charge
...
GateAMDMActor::ImageType4D::Pointer GateAMDMActor::divide4dby3dImage(ImageType4D::Pointer imageToProcess, ImageType::Pointer denominatorImage)
{
  ...
  // std::cout << "in function before division: " << imageToProcess->GetPixel(pixelIndex) << std::endl;
  ...
  for (int i = 0; i < bins; i++)
  {
    ...
    // std::cout << "in function loop before division: " << imageToProcess->GetPixel(pixelIndex) << std::endl;
    // std::cout << "in function loop to divide with: " << denominatorImage->GetPixel(pixelIndex3D) << std::endl;
    // pixelIndex[3] = i; // bin position of the pixel for debugging output
    ...
    // std::cout << "in function loop 3D before division: " << extractFilter->GetOutput()->GetPixel(pixelIndex3D) << std::endl;

    // the paste filter puts the extracted/processed slice back into its original place
    ...
    // std::cout << "after division " << divisionImage->GetPixel(pixelIndex3D) << std::endl;
  ...
  // std::cout << "at end of division function data at voxel: " << imageToProcess->GetPixel(pixelIndex) << std::endl;

  return imageToProcess;
}

GateAMDMActor::ImageType4D::Pointer GateAMDMActor::divide4dImage(ImageType4D::Pointer imageToProcess, ImageType4D::Pointer denominatorImage)
{
  // ImageType4D::IndexType pixelIndex;
  // pixelIndex[0] = 0; // x position of the pixel
  // pixelIndex[1] = 0; // y position of the pixel
  ...
  using DivideImageFilterType = itk::DivideImageFilter<ImageType4D, ImageType4D, ImageType4D>;
  auto divideImageFilter = DivideImageFilterType::New();
  ...
  // std::cout << "in function after division: " << imageToProcess->GetPixel(pixelIndex) << std::endl;
  return imageToProcess;
}
...
void GateAMDMActor::EndSimulationAction()
{
  ...
  // std::cout << "gamma before division: " << cpp_amdm_gamma_image->GetPixel(pixelIndex) << std::endl;
  cpp_amdm_gamma_image = GateAMDMActor::divide4dImage(cpp_amdm_gamma_image, cpp_amdm_delta_image);
  // std::cout << "gamma after division: " << cpp_amdm_gamma_image->GetPixel(pixelIndex) << std::endl;

  // divide delta by restricted edep
  // std::cout << "delta before division: " << cpp_amdm_delta_image->GetPixel(pixelIndex) << std::endl;
  cpp_amdm_delta_image = GateAMDMActor::divide4dby3dImage(cpp_amdm_delta_image, cpp_amdm_restricted_edep_image);
  // std::cout << "delta after division: " << cpp_amdm_delta_image->GetPixel(pixelIndex) << std::endl;
